...
//定义一个函数 计算出itemInfo中所有的haveDone为true的数量
  const count = itemInfo.reduce((acc, cur) => {
    return acc + (cur.haveDone ? 1 : 0);
  }, 0);
  function Footer(){
    return (
      <div className="footer">
        <span>已完成数量为：{count}</span> 
        <span>总数：{itemInfo.length}</span>
      </div>
    );
  }
  return (
